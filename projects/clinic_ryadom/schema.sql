...
-- Включает привязку к проекту/боту и версию политики (для re-consent)
CREATE TABLE IF NOT EXISTS consent (
    user_id     BIGINT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    bot_id      TEXT NOT NULL DEFAULT 'clinic-ryadom',   -- проект/бот, к которому относится согласие
    policy_version TEXT NOT NULL DEFAULT '1.0',           -- версия текста политики
    consent_date TIMESTAMPTZ NOT NULL DEFAULT now(),      -- дата получения согласия
    ...
    health_data_agreed BOOLEAN DEFAULT FALSE, -- согласие на обработку данных о здоровье (спец. категория 152-ФЗ)
...
CREATE TABLE IF NOT EXISTS subscriptions (
    ...
    plan        TEXT DEFAULT 'monthly',      -- monthly (1690/мес) | yearly (17200/год) | pack (разовый)
...
-- Разовый пакет запросов (без подписки): покупка пачки вопросов
CREATE TABLE IF NOT EXISTS question_packs (
    id          BIGSERIAL PRIMARY KEY,
    user_id     BIGINT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    questions   INT NOT NULL DEFAULT 5,       -- сколько вопросов куплено
    used        INT NOT NULL DEFAULT 0,       -- сколько потрачено
    price       NUMERIC(10,2) DEFAULT 0,
    status      TEXT DEFAULT 'active',
    created_at  TIMESTAMPTZ DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_packs_user ON question_packs(user_id, status);

-- Лог рассылок уведомлений: чтобы НЕ спамить (single-fire на событие)
CREATE TABLE IF NOT EXISTS notify_log (
    user_id     BIGINT NOT NULL REFERENCES users(user_id) ON DELETE CASCADE,
    notify_type TEXT NOT NULL,               -- daily_limit | trial_end | subscription_end
    day         DATE NOT NULL DEFAULT CURRENT_DATE, -- номер дня, за который отправлено
    sent_at     TIMESTAMPTZ DEFAULT now(),
    PRIMARY KEY (user_id, notify_type, day)
);
